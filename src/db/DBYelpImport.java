...
public class DBYelpImport {
    ...
    public static void main(String[] args) {
   	 try {
         // Ensure the driver is imported. class loader
   		 Class.forName("com.mysql.jdbc.Driver").newInstance();
   		 Connection conn = null;

   		 try {
   			 System.out.println("Connecting to \n" + DBUtil.URL);
   			 //static method DriverManager
   		 ...
   		 // sequence can't be changed and dropping sequence has to be reverse order of adding 
		 ...
		 sql = "CREATE TABLE users "
				 ...
				 + " PRIMARY KEY ( user_id ))";// primary key is unique for each user_id
		 stmt.executeUpdate(sql);
		 
		 sql = "CREATE TABLE history "
				 + "(visit_history_id bigint(20) unsigned NOT NULL AUTO_INCREMENT, " //create history id as key by auto_increment
				 + " user_id VARCHAR(255) NOT NULL , "
				 + " business_id VARCHAR(255) NOT NULL, "
				 ...
				 + "FOREIGN KEY (user_id) REFERENCES users(user_id))";//user_id in history table points to the user_id in users table
		 stmt.executeUpdate(sql);
		 // Step 3: insert data
   		 // Create a fake user
